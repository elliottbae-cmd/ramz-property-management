...
def get_equipment(store_id: str, active_only: bool = True) -> list[dict]:
    """List equipment for a store, ordered by name. Raises on error (not cached)."""
    sb = get_client()
    query = (
        sb.table("equipment")
        .select("*")
...
@cached_query(ttl=300, default_factory=lambda: None)
...
def get_equipment_for_client(client_id: str) -> list[dict]:
    ...
    non-matching rows after the join. Raises on error (caught by cached_query).
    """
    sb = get_client()
...
def get_equipment_with_details(store_id: str) -> list[dict]:
    ...
    Fully cached — uses bulk queries, no per-item DB calls. Raises on error.
    """
    sb = get_client()
    eq_result = (
        sb.table("equipment")
...
@cached_query(ttl=300, default_factory=dict)
def get_active_warranties_bulk(equipment_ids: tuple | list) -> dict[str, dict]:
    ...
    Raises on error (caught by cached_query) so failures aren't cached.
    """
...
@cached_query(ttl=60, default_factory=lambda: None)
def check_active_warranty(equipment_id: str) -> dict | None:
    ...
    result = (
        ...
        .select("*")
        .eq("equipment_id", equipment_id)
        # Use an actual date string — PostgREST does not evaluate "now()"
        # as a SQL function in a filter value (it compares it literally).
        .lte("start_date", today)
        .gte("end_date", today)
        .limit(1)
        .execute()
...
def create_warranty(data: dict) -> dict | None:
    ...
    *data* should include: equipment_id, warranty_provider, start_date, end_date,
    ...
    try:
        ...
        row = result.data[0] if result.data else None
        if row:
            clear_equipment_cache()  # warranty changes affect warranty + bulk caches
        return row
    except Exception:
        return None
