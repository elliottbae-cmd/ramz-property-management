...
def get_contractors(filters: dict | None = None) -> list[dict]:
    ...
    Returns an empty list on error (but does NOT cache the failure).
    """
    try:
        if filters:
...
def _fetch_contractors(
    ...
    Raises on error so that failures are never cached as empty lists.
    The caller (get_contractors) wraps with try/except for the UI.
    """
    sb = get_client()
    query = (
